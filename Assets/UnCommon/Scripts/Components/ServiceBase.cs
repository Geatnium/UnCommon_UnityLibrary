...
namespace UnCommon
{
    ...
    public abstract class ServiceBase<T> : ComponentBase, IService where T : class, IService
    {
        ...
        // 以降のクラスでは初期化はInit()にまとめる。
        protected sealed override async UniTask OnAwake()
        {
            await base.OnAwake();
            // 自身（派生先のインターフェース）をサービスロケータに登録する
            ServiceLocator.Register(GetComponent<T>());
            // 一回だけ初期化
            InitOnce();
        }
        ...
        // 以降の初期化はInit()でまとめる。
        protected sealed override async UniTask OnStart()
        {
            await base.OnStart();
        }
        ...
        protected override async UniTask OnComponentDestroyed()
        {
            ...
            // 自身が破棄されたらサービスロケータからも除外
            ServiceLocator.Unregister(GetComponent<T>());
        }
